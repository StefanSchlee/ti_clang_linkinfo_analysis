...
## Requirements for Generating linkinfo.xml

The linkinfo.xml file is generated by the TI Clang ARM compiler linker. To enable XML output, you must pass the following linker flags:

- `--xml_link_info`: Enables generation of linkinfo.xml file
- `--gen_xml_func_hash`: Generates refd_ro/refd_rw references between the symbols

**Tested Toolchains:**
- TI CGT ARM v4.0.x
- TI CGT ARM v5.0.0

Contact your TI support or toolchain documentation if you're using a different version to confirm flag compatibility.
...
Copyright (c) 2026 Stefan Schlee
